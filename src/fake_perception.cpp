...
public:
    ConeFilterNode() : Node("cone_filter_node") {
        ...
        this->declare_parameter<std::string>("inner_cones_file", "/home/ubuntu/psd_ws/src/psd_gazebo_sim/psd_gazebo_worlds/world/track_csv/scaled_inner.csv");
        this->declare_parameter<std::string>("outer_cones_file", "/home/ubuntu/psd_ws/src/psd_gazebo_sim/psd_gazebo_worlds/world/track_csv/scaled_outer.csv");

        this->declare_parameter<double>("max_distance", 5);  // [meters]
        this->declare_parameter<double>("max_angle", 180*(M_PI/180));      // [radians]
...
private:
    ...
    void poseCallback(const geometry_msgs::msg::Pose::SharedPtr msg) {
        ...
        for (const auto& cone : cones_) {
            // if(cone.color == 1.0)
            //     RCLCPP_ERROR(this->get_logger(), "Yellow cone \n\tX:  %f\n\tY:  %f\n\tcolor:  %f\n\t", cone.x, cone.y, cone.color);
